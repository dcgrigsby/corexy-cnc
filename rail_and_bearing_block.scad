...
function rail_and_bearing_block_bearing_padding_x(bearing_dia) = bearing_dia*2;
function rail_and_bearing_block_x(rail_dia, bearing_dia) = rail_dia*5 + rail_and_bearing_block_bearing_padding_x(bearing_dia);
...
module rail_and_bearing_block(rail_sep_z, rail_dia, rail_sep_y, bearing_dia, bearing_y, heatset_external_dia, heatset_internal_dia, heatset_z) {
  ...
  difference() {
    ...
    for (shaft_z = [rail_and_bearing_block_z/2 + rail_sep_z/2, rail_and_bearing_block_z/2 - rail_sep_z/2]) {
      ...
      for (sign = [-1, 1]) {
        translate([-rail_and_bearing_block_x/2 - 1, sign*(rail_sep_y/2 +  bearing_y/2 + heatset_internal_dia/2), shaft_z]) {
          rotate([0, 90, 0]) {
            cylinder(d=heatset_internal_dia, h=bearing_dia+1);
            cylinder(d=heatset_external_dia, h=heatset_z+1);
          }
        }
      }
      // rail heatsets
      for (sign = [-1, 1]) {
        translate([rail_and_bearing_block_x/2 - rail_dia*2.5, sign*(rail_and_bearing_block_y/2 + 1), shaft_z]) {
          rotate([sign*90, 0, 0]) {
            cylinder(d=heatset_internal_dia, h=rail_and_bearing_block_z_padding(rail_dia, bearing_dia)/2 + 1);
            cylinder(d=heatset_external_dia, h=heatset_z+1);
          }
        }
      }
...
rail_and_bearing_block(
  ...
  bearing_dia = 15,
  bearing_y = 24,
  heatset_external_dia=4.1,
  heatset_internal_dia=3,
  heatset_z=5
);
*/
